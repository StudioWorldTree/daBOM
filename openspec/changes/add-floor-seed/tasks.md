...
- [x] Every `seedItems` row sets `floor` (`buy` | `assemble` only)
- [x] `SeedItem` type requires `floor` (no optional, no default)
- [x] Buy leaves named: `t4000-som`, preferred carrier `rogue-t5`, PYXIS
      bodies, `rv1126b-turret` / `rv1126b-core`, `poe-switch-at`; resin
      shells `resin-shell-front` / `resin-shell-rear` are `buy` (keep quote)
- [x] Assemble: kits (`kit-prod`, `kit-lab`, `kit-cine`, `kit-hybrid-plant`)
      and assemblies (`asm-thor-sandwich`, `asm-lab-shell`, `asm-sat-mule`,
      `asm-sat-product`)
- [x] Seed backfill in `seed.ts` stays; catalog test proves it is inert on
      the shipped seed. Upsert `set` list includes `floor`.
- [x] Tests: catalog import of `seedItems`/`seedBoms` (fails today);
...
- [x] Catalog test also asserts every `seedBoms` parent/child SKU exists in `seedItems`
